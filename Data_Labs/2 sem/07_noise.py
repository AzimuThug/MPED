...
def main():
    ...
    # img_without_randomNoise = Proccessing.average_filter(img_randomNoise)
    # img_without_impulseNoise = Proccessing.average_filter(img_impulseNoise)
    # img_without_multipleNoise = Proccessing.average_filter(img_multipleNoise)

    # Убираем шум медианным фильтром
    img_without_randomNoise = Proccessing.median_filter(img_randomNoise)
    img_without_impulseNoise = Proccessing.median_filter(img_impulseNoise)
    img_without_multipleNoise = Proccessing.median_filter(img_multipleNoise)

    # Вывод изображений
    fig, ax = plt.subplots(nrows=3, ncols=2, figsize=(10, 7))
    fig.suptitle("Изображение" + name + "с шумами и без (медианный фильтр)", fontsize=15)
